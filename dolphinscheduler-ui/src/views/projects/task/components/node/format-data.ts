...
export function formatParams(data: INodeData): {
    ...
    if (data.taskType === 'JAVA') {
        ...
        if (data.runType === 'JAR' && data.mainJar) {
            taskParams.mainJar = {id: data.mainJar, resourceName: data.name}
        }
    }
    if (
        data.taskType &&
...
export function formatModel(data: ITaskData) {
    ...
    if (data.taskParams?.resourceList) {
        params.resourceList = data.taskParams.resourceList
            .filter((item): item is { id: number } => item.id !== undefined)
            .map(item => {
                return item.id;
            });
